...
const build = async (
  ...
  const codeBundle = await rollup(options);

  await codeBundle.write(options.output as OutputOptions);
...
(async () => {
  ...
  await build('helper/import', ['dist/cache/main.js']);

  await build('index', ['dist/**/*', '!dist/index.js']);
})();
