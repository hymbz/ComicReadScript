...
这是一个因为目前大部分漫画站都不支持双页显示，所以每次遇到 **漫画中的跨页大图被分割成两页** 就很不爽的人为了更好的漫画阅读体验的油猴脚本。为常用漫画站增加了**双页阅读模式**和优化体验的增强功能。

脚本会在网页右下角弹出用于 **进入阅读模式** 的悬浮按钮，其上的快捷按钮用于切换站点增强功能的开启与否。脚本默认开启了自动加载，会在打开页面时自动加载图片并进入阅读模式，也可在这里关闭。
...
2. 之后通过 GreasyFork 安装脚本：[点我](https://sleazyfork.org/zh-CN/scripts/374903-comicread)
