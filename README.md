...
1. 如果页数没被处理掉的话，直接将漫画页数调整到左奇右偶即可~~不过一般漫画的页数都会被处理掉~~
2. 如果漫画图源够好，并且汉化组没有进行白边剪裁的话，在漫画顺序正确的情况下中间会有比较大的白边——即书的中缝
3. 如果出现了页边的注释，只会出现在左右两侧，不会出现在中间

最后就只能靠**违和感**和**经验**了。

关于违和感，漫画看多了后就能在左右顺序出错时感觉到违和感，比如在看漫画时发现好像这页应该在翻页后才出现会更好，不过这个因为是纯凭感觉所以并不能100%肯定。另外，一些漫画在创作时就考虑到了翻页这一动作——比如会特意将比较有冲击性的页面放在翻页后——把这点考虑进去后应该能提高正确率。

关于经验，一些杂志上刊载的漫画会在第一页的右页放剧情梗概和角色介绍，如果汉化组有汉化这页——比如布卡时期的正版百合姬——那这页本身就算是填充页了，不需要开「页面填充」。但如果之后换了汉化组不再汉化这页了，那就肯定得开启了。
