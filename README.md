...
也正因如此，更推荐大家使用本地部署的项目，不抢服务器资源也不需要排队。**一键翻译全部图片**的功能开关只会在使用本地版时可用。具体部署方法可参考 [我的笔记](https://comic-read-docs.pages.dev/本地部署翻译.html)。
...
不过代价是无法在加载时逐步显示图片，要等到整张图加载完毕才能显示。
