...
- 增加了 [ehentai](#ehentai)、[nhentai](#nhentai)的脚本。
- 增加了百合会漫画阅读模式的「上/下一话」功能，仅对有标签的帖子有用。上/下一话为帖子在第一个标签下的上/下一个帖子。
- 增加了动漫之家的「解除吐槽的字数限制」功能。
...
漫画阅读模式的入口在漫画详情页右侧边栏。开启功能后会多出一个「Load comic」按钮，点击后开始加载漫画，加载完毕后按钮变为「Read」，点击开始阅读。

### nhentai相关

#### 匹配漫画

根据漫画标题匹配 nhentai 的本子，结果会以标签的形式显示在详情页的标签栏中。点击标签后，标签菜单有两个选项：「Jump to nhentai」——跳转至对应的 nhentai 网页、「Load comic」——直接在当前页加载 nhentai 的资源，用阅读模式阅读。相比 ehentai，nhentai 的资源加载更快，而且不会消耗配额。

#### 搜索跳转

将点击标签后显示的标签菜单里「Add New Tag」的按钮替换成了「Search nhentai」，点击跳转到 nhentai 的标签结果页。（添加新标签的方法除了标签菜单里的「Add New Tag」外，也可以在关闭标签菜单时在相同位置显示的 new tag field 添加，所以并不会影响正常使用）
...
漫画阅读模式的入口在漫画详情页收藏按钮的旁边。开启功能后会多出一个「Load comic」按钮，点击后开始加载漫画，加载完毕后按钮变为「Read」，点击开始阅读。
