...
/** 是否为单页模式 */
const isOnePageMode = helper.createRootMemo(() => pageNum() === 1 || store.option.scrollMode.enabled && !store.option.scrollMode.doubleMode || store.isMobile || store.imgList.length <= 1);
...
const switchOnePageMode = () => {
  setOption((draftOption, state) => {
    if (draftOption.scrollMode.enabled) draftOption.scrollMode.doubleMode = !draftOption.scrollMode.doubleMode;else {
...
const handleSwitchChapter = setManga => {
  ...
  for (const element of helper.querySelectorAll('a, button')) {
    // 删除可能混在其中的特殊符号
    const text = element.textContent?.replaceAll(/[<>()《》（）「」『』]/g, '').trim();
    if (!text) continue;
...
const handleVersionUpdate = async () => {
  ...
  if (helper.lang() === 'zh') {
    Toast.toast(() => /* eslint-disable i18next/no-literal-string */[(() => {
      ...
      return _el$;
    })(), web.template(\`<h3>修复\`)(), web.template(\`<ul><li>适配新版 manga-image-translator\`)(), web.createComponent(VersionTip, {
      v1: version,
      v2: '10.8.0',
      get children() {
...
(async () => {
  ...
  web.render(() => {
    ...
    const handleClick = e => {
      ...
      prompt(helper.t('other.page_range')) ?? `1-${totalImgNum}`);
      // 删掉当前的图片列表以便触发重新加载
      setComicMap('', 'imgList', undefined);
    };
    return (() => {
