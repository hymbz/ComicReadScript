...
对于支持站点以外的网站，如果能够在一个页面内显示所有漫画页图片一屏到底的话，可以使用「[简易阅读模式](#简易阅读模式)」直接阅读，否则的话欢迎 [反馈](issues) 申请增加支持。
...
默认会开启「记住当前站点」功能，在之后再次打开站点时自动使用简易阅读模式，享受和其他支持站点一样的体验。可通过右下角悬浮按钮上的快捷按钮关闭。
