...
> ehentai 图片链接会在一段时间后过期失效，因此建议开启「始终加载所有图片」，确保在过期前加载好所有图片，避免在过期后需要重新获取新链接增加耗时。
> 如果还需要下载和翻译的话，建议开启「[图像识别](#图像识别)」功能，避免在下载和翻译时出错。
...
关联其他站点的漫画源，使用外站源来加载图片，避免消耗 ehentai 的配额，同时或许也能用更快的速度加载图片。

结果会以标签的形式显示在标签列表中，鼠标悬停在标签上可以看到外站漫画的具体标题，点击标签后会出现跳至外站或直接加载的选项，也可以直接右键标签点击「在新标签页中打开」来跳至外站。

目前支持 nhentai 和 hitomi。

> nhentai 的反爬风控会定期调整，有时候会严格到即使已经在 nhentai 登录了也会出错，只能嗯等阶段性调整过去
...
同时顺便对 My Tags 和列表页显示的标签按照颜色、命名空间、权重等信息进行排序。
...
> 开启此功能后可能出现一整页的漫画都被屏蔽的情况，建议搭配无限滚动
